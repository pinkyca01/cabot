...
// Owner Menu
        ...
        case 'setfoto': case 'setthumb':
				...
				break
        case 'upswtext':
                if (!caa.key.fromMe) return nuy.sendMessage(from, '「 KHUSUS OWNER 」',MessageType.text, { quoted: caa, contextInfo: { forwardingScore: 999, isForwarded: true}})
		...
		case 'upswimg':
				...
				nuy.updatePresence(from, Presence.composing)
				if (isQuotedImage) {
				const swsw = isQuotedImage ? JSON.parse(JSON.stringify(caa).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : caa
				    pinky = await nuy.downloadMediaMessage(swsw)
				    nuy.sendMessage('status@broadcast', pinky, image, { caption: `${q}` })
				}
				bur = `Sukses Upload Story Image dengan Caption: ${q}`
				nuy.sendMessage(from, bur, text, { quoted: caa })
				break
		case 'upswvideo':
				if (!caa.key.fromMe) return nuy.sendMessage(from, '「 KHUSUS OWNER 」',MessageType.text, { quoted: caa, contextInfo: { forwardingScore: 999, isForwarded: true}})
				nuy.updatePresence(from, Presence.composing)
				if (isQuotedVideo) {
				const swsw = isQuotedVideo ? JSON.parse(JSON.stringify(caa).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : caa
					pinky = await nuy.downloadMediaMessage(swsw)
